...
- **Hard bounds before parsing:** each provider considers at most 128 matching
  ...
  The database (plus WAL/SHM) is bounded by its own 512 MiB ceiling rather
  than the shared 64 MiB transcript allowance, so a large opencode archive
  still lists its sessions without starving the file-based providers, and
  every auxiliary file receives the same regular-file check.
...
```bash
omarchy plugin add https://github.com/engineer-hamas/omarchy-agents-sessions.git --enable
```
...
```bash
omarchy plugin remove engineer-hamas.agents-sessions
```

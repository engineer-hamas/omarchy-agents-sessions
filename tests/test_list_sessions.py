...
class ScannerTests(unittest.TestCase):
    ...
    def test_opencode_only_selects_rows_without_a_parent(self):
        ...
        # A store whose footprint exceeds the shared transcript byte budget is
        # not refused: SQLite stores are bounded by their own SQLITE_FILE_MAX
        # ceiling and the wall-clock deadline, not the shared byte allowance,
        # so a large opencode archive still lists its sessions.
        oversized = SESSIONS.ScanBudget(seconds=1, max_bytes=1)
        rows = list(SESSIONS.scan_opencode(oversized))
    ...
    def test_oversized_json_and_fifo_database_are_skipped(self):
        ...
        # The database itself is still capped: a store at or over
        # SQLITE_FILE_MAX is refused before any query runs.
        os.unlink(str(database) + "-wal")
        database.unlink()
        con = sqlite3.connect(database)
        con.execute("create table session (id text)")
        con.close()
        os.truncate(database, SESSIONS.SQLITE_FILE_MAX + 1)
        big_budget = SESSIONS.ScanBudget(seconds=1)
        self.assertEqual(list(SESSIONS.scan_opencode(big_budget)), [])
        self.assertIn("database size limit", big_budget.reasons)
